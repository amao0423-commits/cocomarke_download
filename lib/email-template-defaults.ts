/** 新規メールテンプレ作成時の既定件名（site_settings 未設定時のフォールバック） */
export const DEFAULT_EMAIL_TEMPLATE_SUBJECT =
  '【COCOマーケ】資料ダウンロードありがとうございます。';

/** テンプレ新規作成時に挿入する既定本文（HTML）。本文のテキスト部分だけ書き換えればOK。ボタンは {{documentButtons}} で紐づけた資料が自動挿入。ロゴは {{emailLogoUrl}}（送信時に絶対URLまたは埋め込みに変換）。 */
