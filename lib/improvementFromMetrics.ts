...
export function buildImprovementFromMetrics(input: MetricsInput): string[] {
  ...
  if (followers && followers > 0 && (avgLike != null || avgComment != null)) {
    ...
    messages.push(
      ...
      }件）。${judge}。現在のInstagramでは、リーチを最も左右する指標が「いいね」から『送信数（DMでのシェア）』へと移っており、Instagram責任者のMosseri氏も、送信は新規リーチにおいて“いいねの約3〜5倍”の重みを持つと明言しています。そのため今後は「思わず友だちに送りたくなる」保存・共有価値のある投稿設計（保存版のノウハウ、あるあるネタ、チェックリスト等）を軸に据えてください。`
    );
  }
  ...
  // 6. ハッシュタグ（現行方針：最大5個・リーチ効果なし・分類/検索の手がかり）
  messages.push(
    'ハッシュタグは現在、1投稿あたり最大5個までです。かつてのように数を増やしてもリーチは伸びず（Instagram責任者Mosseri氏も「ハッシュタグはリーチ向上の主要因ではない」と明言）、役割は投稿内容の分類と検索の手がかりに限られます。そのため、関連性の高い3〜5個に厳選し、大量付けはやめてください。ハッシュタグに頼るより、キャプション本文に検索されたいキーワードを自然に盛り込む「キーワード検索対策」のほうが、現在は発見されやすさに直結します。'
  );

  // 7. ストーリーズ・DM（親密度シグナルと送信を後押し）
  messages.push(
