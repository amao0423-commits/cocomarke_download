...
const PLANS = {
  like: { name: "いいね代行", price: "9,800", why: "ターゲット層への自動いいねで認知の入口をつくり、投稿の反応を底上げします。まず低コストで「見つけてもらう」土台づくりに最適です。" },
  boost: { name: "発見表示ブースト", price: "19,800", why: "おすすめ・発見タブへの露出を強化し、新規リーチを最大化します。新規に知られたい方に最も効果的なプランです。" },
  set: { name: "セットプラン", price: "24,980", why: "いいね代行と発見表示ブーストを組み合わせ、認知拡大とリーチを両立。伸び悩みを抜け出したい方に人気No.1の組み合わせです。" },
  rank: { name: "リスト上位表示", price: "14,800", why: "狙ったキーワードでの検索上位を押し上げ、「エリア×業種」で見つけられる状態をつくります。指名・地域集客の強化に。" },
  premium: { name: "プレミアム", price: "49,800", why: "投稿代行を含めた専任担当が、戦略から実行までまるごと伴走。手間をかけずに、任せるだけで成果につながる運用が回ります。" },
} as const;
...
function recommend(ans: Record<QKey, string>): { plan: PlanKey; note?: string } {
  ...
  if (ans.issue === "reach") plan = "boost";
    ...
    plan = "set";
    note = "投稿制作までお任せになりたい場合は、投稿制作オプションの追加や、専任担当が伴走するプレミアム（要相談）もご案内できます。";
  } else plan = "set";
